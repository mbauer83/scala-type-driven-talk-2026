...
export
Show (Money c) where
  show {c} (MkMoney cents) = show cents ++ "c " ++ currencyCode c
...
classifyRisk order =
  let total = orderTotalCents order
  in case order.paymentMethod of
       Invoice _ => HighRisk
       Wallet _ =>
         if lteNat total 20000 then LowRisk else MediumRisk
       Card _ =>
         if lteNat total 15000 then LowRisk
         else if lteNat total 80000 then MediumRisk
         else HighRisk

export
riskReason : Order n c -> String
riskReason order =
  let total = show (orderTotal order)
  in case classifyRisk order of
       LowRisk => "small order total " ++ total ++ " via " ++ paymentMethodLabel order.paymentMethod
       MediumRisk => "higher order total " ++ total ++ " requires customer challenge"
       HighRisk => "high-value or invoice flow " ++ total ++ " requires manual review"

public export
record Assessment (n : Nat) (c : Currency) where
...
export
assessOrder : Order n c -> Assessment n c
assessOrder order = MkAssessment order (classifyRisk order) (riskReason order)
...
export
approvalSummary : Approval level -> String
approvalSummary AutoApproved = "auto-approved"
approvalSummary (ThreeDSApproved proof) =
  "3ds-proof:" ++ proof.challengeId ++ " shift=" ++ show proof.liabilityShift
approvalSummary (ReviewerApproved approval) =
  "reviewer=" ++ approval.reviewer ++ " note=" ++ approval.note
...
export
authorize : (assessment : Assessment n c) -> Approval assessment.level -> AuthorizedPayment n c
