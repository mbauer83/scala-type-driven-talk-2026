...
sentences it trips the `tricolon` rule — correctly by the letter, wrongly in
spirit: a syllogism is a three-part form, not a cadence trick. Semicolons also
read better, because the three parts are one unit.

DELIVERY
The syllogism is on the slide in two columns; walk the left one, then point at
the right. The beat that matters is *validity comes from the shape* — pause after
it.

WHY THE OLD LINE WENT (Part 10)
It read *the content is gone, the argument survived*. MB: overused-AI phrasing,
and it is — two short declaratives in apposition doing the work a claim should do.
The replacement states the claim instead of performing it, and its second half
carries the F1 correction: a shape is checkable by inspection, so mechanical
checking starts here rather than with Boole's algebra.

C13 CHECK (Part 8)
Nothing here is yet about programs or types. This beat establishes only that
validity is structural. Do not reach forward to code — `A1-connectives` does that,
