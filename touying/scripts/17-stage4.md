...
4b. NAME THE BLOCK: authorizeReview, top right — the reviewer's approval is an
    argument, and it hands back Payment<Authorized>
4c. THE LADDER, below it: Payment<Initiated>, <Authorized>, <Captured>,
    <Refunded> — one parameter, four values, capture names the one it takes
...
is a function that performs the check and hands back the evidence — on the right,
that function is `authorizeReview`.
...
And Charlie's refund is the last rung of that same ladder — the rung his shortcut
went around.
