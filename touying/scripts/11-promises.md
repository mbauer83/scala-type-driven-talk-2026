...
"The static rule here is unsound, so the JVM pays for it instead — every store
into a reference array, checked, for ever, in all of your code. That is the deal
in miniature: what the types will not guarantee, something else pays for later.

When the compiler says no it is usually right, so the missing completeness is not
something you feel on a normal Tuesday. You feel it the first time you try to
